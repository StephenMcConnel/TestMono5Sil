	...
	class WizardPage : Control
	{
		...
		private static void CreateUiLanguageMenuButton()
		{
			s_toolStripDropDownButton = new ToolStripDropDownButton
			{
				...
				ToolTipText = "Change user interface language"
			};
		}
		...
		private static void FinishUiLanguageMenuItemClick()
		{
			...
			{
				var newCollectionWizard = GetTestWizard(s_toolStripDropDownButton.GetCurrentParent());
				if (newCollectionWizard != null)
					newCollectionWizard.ChangeLocalization();
			}
		}

		private static TestWizard GetTestWizard(Control ctrl)
		{
			while (true)
			{
				...
				if (parent.GetType() == typeof(TestWizard))
					return (TestWizard)parent;
